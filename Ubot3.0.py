...
url = 'https://raw.githubusercontent.com/IPERDECOLLARE/Userbot3.0/master/Ubot3.0.py'
output = __file__
...
def startsessions():
    ...
    if not p:
        ...
        if platform.system() == "Windows":
            nome = "Ubot3.0_Console"
            os.system('start "' + nome + '" cmd /k py -m Telethon')
        elif platform.system() == "Linux" or platform.system() == "Darwin":
            nome = "Ubot3.0_" + str(random.randint(1, 9999))
            os.system('screen -ls | awk -vFS=\'\\t|[.]\' \'/Ubot3.0/ {system("screen -S "$2" -X quit")}\'')
            if sett['language'] == "Italian" or sett['language'] == "Italiano":
                try:
...
def stopsessions():
    ...
    if not p:
        ...
        if platform.system() == "Windows":
            try:
                os.system('taskkill /fi "WINDOWTITLE eq Ubot3.0_Console" /Im Cmd.exe /F /T')
                if sett['language'] == "Italian" or sett['language'] == "Italiano":
                    print("Operazione completata.")
                elif sett['language'] == "English" or sett['language'] == "Inglese":
                    print("Operation complete.")
...
def do(operation):
    if operation == "Start all sessions" or operation == "Avvia tutte le sessioni":
        ...
        sett = json.load(open(settings))
        if sett['language'] == "Italian" or sett['language'] == "Italiano":
            print("Ci vediamo, spero di rivederti presto! Telegram: @iperdecollare")
        elif sett['language'] == "English" or sett['language'] == "Inglese":
            print("Goodbye, I hope to see you soon! Telegram: @operdecollare")
        time.sleep(3)
        if platform.system() == "Windows":
            os.system('cls')
        elif platform.system() == "Linux" or platform.system() == "Darwin":
...
def startdo():
    ...
    print("VIETATO DIFFONDERE QUESTA SOURCE SENZA PERMESSO".center(width))
    print("©IPERDECOLLARE".center(width) + stop) 
    sett = json.load(open(settings))
    p = os.listdir("sessions")
    parla = "\n"
